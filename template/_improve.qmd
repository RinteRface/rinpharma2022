...
dracula_card(
  ...
  dracula_text(
    ...
    weight = "bold",
    size = "lg",
    "For learning purposes we disable load test, profiling, ..."
  )
)
```
...
- name: Audit app 🏥
  shell: Rscript {0}
  run: shinyValidator::audit_app(output_validation = TRUE)
```
...
## Performance: Code profiling (2/2)

:::: {.columns}

::: {.column width="50%"}
Enable profiling in CI/CD:

```yaml
- name: Audit app 🏥
  shell: Rscript {0}
  run: shinyValidator::audit_app(load_testing = FALSE, check_reactivity = FALSE)
```

Commit, push, wait and review the profiling tab ...
:::

::: {.column width="50%"}
TO DO: add profiling widget ...
:::

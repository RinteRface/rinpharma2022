...
Modify the script created in `./app_audit.R`.
...
Run `./app_audit.R` and have a look at the coverage tab.
...
- Control inputs with `$set_inputs()`. 
- Take screenshot with `$get_screenshot()`.

Run `./app_audit.R` and have a look at the screenshots
...
Enable output check in `./app_audit.R`:
...
4. Run `./app_audit.R` script and look at the outputs tab.
:::
...
Run `./app_audit.R` and have a look at the profiling tab.
